...
def extract_batch(matrix, center, w, h):
    ...
    for i in range(w2):
        for j in range(h2):
            for k in range(3):
                batch[k][i][j] = matrix[center[0] - w2 + i+1][center[1] - h2 + j+1][k]
                batch[k][w2+i][h2+j] = matrix[center[0] + i+1][center[1] + j+1][k]
                batch[k][i][h2+j] = matrix[center[0] -w2 + i+1][center[1] + j+1][k]
                batch[k][w2+i][j] = matrix[center[0] + i+1][center[1] -h2+ j+1][k]
    return batch
...
def VGG_16(input_size):
    ...
    model.add(Dense(input_size * input_size * 2, activation='relu'))
    # model.add(Dropout(0.5))
...
for img_name, mask in test_data:
    ...
    for x in range(x_size):
        for y in range(y_size):
            ...
            for i in range(SIZE):
                for j in range(SIZE):
                    if prediction[l][i*SIZE + j] > 0.5:
                        img_reconstruct[x*x_size + i][y * y_size + j] = 1
                    else:
